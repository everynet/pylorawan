...
def calculate_channels(table_datarates: typing.Dict[int, DataRate], freq_offset_hz: int) -> typing.List[Channel]:
    freq_offset_khz = freq_offset_hz // 1000
    ...
    return [
        ...
        Channel(
            index=0,
            frequency_khz=923200 + freq_offset_khz,
            datarates=datarates,
            fixed=True,
        ),
        Channel(
            index=1,
            frequency_khz=923400 + freq_offset_khz,
            datarates=datarates,
            fixed=True,
        ),
        # # Extra channels
        Channel(
            index=2,
            frequency_khz=923000 + freq_offset_khz,
            datarates=datarates,
            fixed=False,
        ),
        Channel(
            index=3,
            frequency_khz=923600 + freq_offset_khz,
            datarates=datarates,
            fixed=False,
        ),
        Channel(
            index=4,
            frequency_khz=923800 + freq_offset_khz,
            datarates=datarates,
            fixed=False,
        ),
        Channel(
            index=5,
            frequency_khz=924000 + freq_offset_khz,
            datarates=datarates + [table_datarates[6]],
            fixed=False,
        ),
        Channel(
            index=6,
            frequency_khz=924200 + freq_offset_khz,
            datarates=datarates,
            fixed=False,
        ),
        Channel(
            index=7,
            frequency_khz=924400 + freq_offset_khz,
            datarates=[table_datarates[7]],
            fixed=False,
        ),
    ]
...
class AS923_2(Band):
    # AS923-2 Group params
    FREQ_OFFSET_HZ = -1.80 * 10**6
    ...
    table_datarates: typing.Dict[int, DataRate] = {
        ...
        15: RFUDataRate(index=15),  # DR15 and TXPower15 are defined in the LinkADRReq MAC command of the LoRaWAN® 1.0.4
    }
    ...
    table_tx_power: typing.Dict[int, typing.Union[int, consts.RFU]] = {
        ...
        13: consts.RFU(),
        14: consts.RFU(),
        15: consts.RFU(),  # DR15 and TXPower15 are defined in the LinkADRReq MAC command of the LoRaWAN® 1.0.4
    }
    ...
    table_maximum_payload_size: typing.Dict[int, MaxPayloadSizeWithDwell] = {
        ...
        7: MaxPayloadSizeWithDwell(dr=5, n_dwell_0=222, n_dwell_1=222),
        # 8: Not defined,
        # 9: Not defined,
        # 10: Not defined,
        # 11: Not defined,
    ...
    rx2_frequency = 9232000 + FREQ_OFFSET_HZ // 100  # freq (in hundreds of Hz) = rx2_frequency (in KHz) * 10
    ...
    uplink_channels = downlink_channels = calculate_channels(table_datarates, FREQ_OFFSET_HZ)

    def get_downlink_datarate(
        self, uplink_datarate: DataRate, rx1_datarate_offset: int, dwell_time: int = 0
    ) -> DataRate:
        ...
        downlink_dr_index = max(
            dwell_time and 2,
            min(
                7,
                uplink_datarate.index
                - (rx1_datarate_offset % 6 + rx1_datarate_offset // 6) * (-1) ** (rx1_datarate_offset // 6),
            ),
        )
